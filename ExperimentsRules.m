...
clearvars ContextPsnr Psnr  Labeling RulesPsnr RulesClusterCompre
load ExpImages.mat

description='Experimrnts for diffrent rules';
%% --------------------------- PARAMETERS ------------------------------
...
for l=1:length(lambda_array)
    ...
    [samp_iter_Output   ]=removenoise(double(Image),Noise,Analysis.iterations(samp_iter).AssignVec2);
    ...
    [samp_RI,samp_MH]   = RandIndex(Analysis.iterations(samp_iter).AssignVec2 (:),ORACLE(:));
%% structures to save
    temppsnr=struct('lambda',lambda_array(l),'CoOcThr',CoOcThr_array(thr),...
        'MaxIter',Parameter.spatial.MaxIter,'psnr2',result2,'samp_iter',samp_iter,'samp_result',samp_iter_result2);
    tempCmpClst=struct('lambda',lambda_array(l),'CoOcThr',CoOcThr_array(thr),...
...
Psnr=struct('wsize',wsize,'sigma',sigma,'Psnr',result);
        % ORACLE
        ...
        full_Data(i)=struct('ImageName',I(i).name,'Psnr',Psnr,...
