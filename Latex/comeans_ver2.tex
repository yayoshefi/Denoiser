...
We use the labels statistics to compute the conditional probability between two labels.
$$ \color{red}
 P(K|J)=\Pr\{ \widehat{L}_{(\mathcal{N}(i))}=K|\widehat{L}_{(i)}=J \}=	\sum_{j}P _j(K) \ \ s.t. \begin{cases}
&j \in {\mathcal{N}(i)}\\
&\widehat{L}(i)=J
\end{cases}  $$
...
A more formal definition should be:  $ P(K|J)=\sum_{i} P _j(K) \ \ s.t. \begin{cases}
&j \in {\mathcal{N}(i)}\\
&\widehat{L}(i)=J
\end{cases}  $
