...
bins=1000;
miniwindow=Analysis.miniwindow;
...
p=logical(padarray(ones(m-miniwindow+1,n-miniwindow+1),[floor(miniwindow/2),floor(miniwindow/2)]));
% use p to convert all pathes Images to only inner Image (after miniwindow)
...
LabelPatch=im2col(AssignImg,[miniwindow,miniwindow],'sliding');
...
ylabel(['# similar patches in ',num2str(miniwindow),' window']);
xlabel('Var(signal)')
...
legend ('Mean','Median','std','Location','NorthEastOutside')
ylabel('DC change after Clean]');xlabel(['# similar patches in ',num2str(miniwindow),' window']);
